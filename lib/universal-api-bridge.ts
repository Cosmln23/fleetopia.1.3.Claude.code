...
export const API_PROVIDERS: APIProviderConfig[] = [
  // Universal GPS Provider
  {
    category: 'gps',
    provider: 'universal_gps',
    name: 'Universal GPS Connector',
    description: 'Connect ANY GPS tracking system with a simple configuration. Works with all REST APIs.',
    tier: 'free',
    credentialsRequired: ['Flexible - API Key, Bearer Token, or Basic Auth'],
    rateLimit: { requests: 999999, period: 'month' },
    costEstimate: { free: 'Unlimited - depends on your GPS provider' },
    setupComplexity: 'easy',
    documentation: '/docs/universal-gps-connector',
    compatibleModels: [
      'Any REST API with GPS endpoints',
    ...
    setupInstructions: [
      'Choose a GPS provider template or configure custom API endpoints',
      'Enter your GPS API credentials (API key, token, or username/password)',
      'Map data fields (latitude, longitude, vehicle ID, timestamp)',
      'Test the connection to verify everything works',
      'Save configuration and start tracking your vehicles in real-time'
    ],
    integrationFlow: 'Choose Template → Configure API → Map Data Fields → Test Connection → Start Tracking',
    requiredFields: ['Base API URL', 'Authentication credentials', 'Data field mapping'],
    supportedFeatures: [
      'Universal API compatibility',
      'Flexible authentication (API Key, Bearer, Basic)',
